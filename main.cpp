...
class ppnlm_denoiser {
    ...
    const int rel_max = (int) pow(2 * kernel_size + 1, 2);
    ...
    float upfusePixel(Mat &finer, Mat &coarser, int local_y, int local_x, float &weight_sum, Mat &deltas) {
        ...
        Mat local_patch = getPatch(finer, 2 * (local_y / 2), 2 * (local_x / 2));

        int min_y, min_x, max_y, max_x;
        computeMinMax(coarser, local_y / 2, local_x / 2, min_y, min_x, max_y, max_x, kernel_size);
        ...
        for(int y = min_y; y < max_y; y++) {
            for(int x = min_x; x < max_x; x++) {
                ...
                if(store_deltas && local_y % 2 == 0 && local_x % 2 == 0) {
                    ...
                    Mat other_patch = getPatch(finer, y * 2, x * 2);
                    delta = calculateDelta(local_patch, other_patch);
                    assert((delta >= 0 && delta <= 1));
                }
                ...
                weight_sum += weight;
                pixel_sum += weight * coarser.at<float>(y, x);
            }
        }
    ...
    void clampRels(Mat& rels) {
        for(int i = 0; i < rels.rows; i++) {
            for(int j = 0; j < rels.cols; j++) {
                float val = rels.at<float>(i, j);
                float clamped_val = fmin(fmax(0, val - lower_bound), rel_max - upper_bound);
                float normalized_val = clamped_val / divider;
                assert((normalized_val >= 0 && normalized_val <= 1));
                rels.at<float>(i, j) = normalized_val;
            }
        }
    }
...
int main() {
    ...
    string image_path = "images/source/cat/cat_0.10.png";
    ...
    // write levels as images
    //ppnlm.writeLevels();

    // write output image
    float_output.convertTo(output, CV_8U, 255.0);
    imwrite("images/result/cat.png", output);
